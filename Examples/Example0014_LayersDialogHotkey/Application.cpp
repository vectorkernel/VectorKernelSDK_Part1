...
void Application::RebuildOverlayLines()
{
    ...
    m_overlayVertices.reserve((crosshairLines.size() * 2) + (m_selection.GetSelectionCount() * 12) + 1024);
...
void Application::AppendGlowOverlayForLine(const LineEntity& line, const glm::vec4& color, float offsetPx)
{
    ...
    if (len > 1.0e-3f)
    {
        ...
        const glm::vec2 a0 = a + off;
        const glm::vec2 b0 = b + off;
        const glm::vec2 a1 = a - off;
        const glm::vec2 b1 = b - off;

        m_overlayVertices.push_back({ glm::vec3(a0, 0.0f), color });
        m_overlayVertices.push_back({ glm::vec3(b0, 0.0f), color });

        m_overlayVertices.push_back({ glm::vec3(a1, 0.0f), color });
        m_overlayVertices.push_back({ glm::vec3(b1, 0.0f), color });
    }
}
