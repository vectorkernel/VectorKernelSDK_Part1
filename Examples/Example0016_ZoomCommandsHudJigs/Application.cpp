...
namespace
{
    hershey_font* TryLoadCommandFontFromKnownLocations()
    {
        ...
        if (!exeDir.empty())
        {
            candidateDirs.push_back(exeDir / "hershey-fonts");
            candidateDirs.push_back(exeDir.parent_path() / "RenderCoreLib" / "hershey-fonts");
            candidateDirs.push_back(exeDir.parent_path().parent_path() / "RenderCoreLib" / "hershey-fonts");
            candidateDirs.push_back(exeDir.parent_path().parent_path().parent_path() / "RenderCoreLib" / "hershey-fonts");
        }
        candidateDirs.push_back(fs::path("RenderCoreLib") / "hershey-fonts");
        candidateDirs.push_back(fs::path("..") / "RenderCoreLib" / "hershey-fonts");
        candidateDirs.push_back(fs::path("..") / ".." / "RenderCoreLib" / "hershey-fonts");

        for (const fs::path& dir : candidateDirs)
        {
            const fs::path probe = dir / "futural.jhf";
            if (!fs::exists(probe, ec))
                continue;

            _putenv_s("HERSHEY_FONTS_DIR", dir.string().c_str());
            if (hershey_font* font = hershey_font_load("futural"))
            {
                std::cout << "Loaded Hershey command font from: " << dir.string() << "\n";
                return font;
            }
        }

        std::cout << "WARNING: Could not locate hershey-fonts/futural.jhf for HersheyCommandWindow.\n";
        return nullptr;
    }
}

Application::Application()
    : m_commandWindow(*this)
{
}
...
void Application::Initialize()
{
    ...
    glGenBuffers(1, &m_overlayLineVbo);
    glGenBuffers(1, &m_overlayTriVbo);
    ...
    m_commandFont = TryLoadCommandFontFromKnownLocations();
    if (m_commandFont != nullptr)
    {
        m_commandWindow.SetFont(m_commandFont);
    }
    ...
    m_layersDialog.Create(m_instance, m_mainHwnd, &m_layerTable, CW_USEDEFAULT, CW_USEDEFAULT, 760, 420);

    std::cout << "Zoom commands demo controls:\n"
              << "  Hover over dragon segments to highlight them (HUD overlay).\n"
              << "  X  : cycle crosshair mode\n"
              << "  B  : cycle background preset\n"
              << "  Ctrl+L : toggle Layers dialog\n"
              << "  zoomex : zoom extents through the Hershey command window\n"
              << "  zoomw  : start a temporary zoom window HUD jig\n"
              << "  Ctrl+1 : toggle Properties dialog\n"
              << "  R  : reset view\n"
...
{
    RebuildGridIfNeeded();
    m_commandWindow.Update(inDeltaTime);
    RebuildOverlayLines();
    UploadOverlayToGpu();
}
...
void Application::Render(const ClientSize& clientSize)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_overlayTriVbo);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, pos)));
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, color)));
    glDrawArrays(GL_TRIANGLES, 0, static_cast<GLsizei>(m_overlayTriVertices.size()));

    glBindBuffer(GL_ARRAY_BUFFER, m_overlayLineVbo);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, pos)));
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, color)));
...
{
    if (m_program == 0 && m_sceneVbo == 0 && m_overlayLineVbo == 0 && m_overlayTriVbo == 0 && !m_book)
        return;
    ...
    if (m_overlayLineVbo)
    {
        glDeleteBuffers(1, &m_overlayLineVbo);
        m_overlayLineVbo = 0;
    }

    if (m_overlayTriVbo)
    {
        glDeleteBuffers(1, &m_overlayTriVbo);
        m_overlayTriVbo = 0;
    }
    ...
    m_overlayTriVertices.clear();

    if (m_commandFont != nullptr)
    {
        hershey_font_free(m_commandFont);
        m_commandFont = nullptr;
    }

    m_layersDialog.Destroy();
...
void Application::OnResize(const ClientSize& clientSize)
{
    ...
    m_commandWindow.SetViewportSize(clientSize.width, clientSize.height);
    FitDragonToClient();
    RebuildGridIfNeeded();
...
void Application::OnLMouseButtonDown(int x, int y)
{
    ...
    if (m_commandWindow.HandleLeftButtonDown(x, y))
    {
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
    ...
    if (IsZoomWindowHudJigActive())
    {
        const glm::vec2 world = m_panZoom.ScreenToWorld(x, y);
        m_hudJigRect.Begin(world);
        m_hudJigRect.Update(world);
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
    }
...
void Application::OnLMouseButtonUp(int x, int y)
{
    ...
    m_commandWindow.HandleLeftButtonUp();
    m_rawMouseX = x;
    m_rawMouseY = y;

    if (IsZoomWindowHudJigActive())
    {
        m_hudJigRect.Update(m_panZoom.ScreenToWorld(x, y));
        if (m_hudJigRect.HasDragRect())
        {
            const glm::vec2 worldMin(glm::min(m_hudJigRect.startWorld.x, m_hudJigRect.currentWorld.x),
                                     glm::min(m_hudJigRect.startWorld.y, m_hudJigRect.currentWorld.y));
            const glm::vec2 worldMax(glm::max(m_hudJigRect.startWorld.x, m_hudJigRect.currentWorld.x),
                                     glm::max(m_hudJigRect.startWorld.y, m_hudJigRect.currentWorld.y));
            ZoomToWorldWindow(worldMin, worldMax);
            AddCommandHistory("ZOOMW complete.");
        }
        else
        {
            AddCommandHistory("ZOOMW cancelled: window too small.");
        }
...
void Application::OnRMouseButtonDown(int x, int y)
{
    ...
    if (m_commandWindow.IsVisible() && m_commandWindow.IsHovered())
    {
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
...
void Application::OnMouseMove(int x, int y, bool leftButtonDown, bool rightButtonDown, bool shiftDown)
{
    ...
    const bool commandConsumed = m_commandWindow.HandleMouseMove(x, y);
    m_rawMouseX = x;
    m_rawMouseY = y;
    ...
    {
        RefreshSnappedMousePosition();
        if (m_hudJigRect.dragging && leftButtonDown)
            m_hudJigRect.Update(world);
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
...
void Application::OnMouseLeave()
{
    ...
    m_commandWindow.HandleMouseLeave();
...
{
    if (m_commandWindow.HandleMouseWheel(x, y, wheelDelta))
    {
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
...
{
    const bool ctrlHeld = IsCtrlDown();
    const bool shiftHeld = (GetKeyState(VK_SHIFT) & 0x8000) != 0;
    if (m_commandWindow.HandleKeyDown(vk, ctrlHeld, shiftHeld))
    {
        RebuildOverlayLines();
        UploadOverlayToGpu();
        return;
    ...
    case 'L':
        if (ctrlHeld)
        {
            m_layersDialog.Toggle();
            m_layersDialog.Refresh();
            std::cout << "Layers dialog: " << (m_layersDialog.IsVisible() ? "shown" : "hidden") << "\n";
        }
        break;
    ...
    case VK_ESCAPE:
        ...
        m_selection.ClearHover();
        m_selection.CancelBox();
        CancelActiveHudJig("HUD jig cancelled.");
        SyncPropertiesWindow();
        m_pendingLeftCrossing = false;
        m_pendingRightBox = false;
        if (m_panWithRightDrag)
...
void Application::OnLayerDialogStateChanged()
{
    m_selection.ClearHover();

    std::vector<std::size_t> retainedSelection;
    retainedSelection.reserve(m_selection.GetSelectionCount());
    for (std::size_t id : m_selection.GetSelectedEntities())
    {
        const LineEntity* line = FindLineById(id);
        if (line != nullptr && line->visible && m_layerTable.IsLayerSelectable(line->layerId))
            retainedSelection.push_back(id);
    }

    m_selection.ClearSelection();
    m_selection.AddRange(retainedSelection);
...
void Application::RebuildOverlayLines()
{
    ...
    m_overlayLineVertices.reserve((crosshairLines.size() * 2) + (m_selection.GetSelectionCount() * 12) + 1024);
    m_overlayTriVertices.reserve(1024);
    ...
    for (const LineEntity& line : crosshairLines)
    {
        ...
        m_overlayLineVertices.push_back({ line.p0, line.color });
        m_overlayLineVertices.push_back({ line.p1, line.color });
    }
    ...
    std::vector<AppCore::UiVertex> commandLines;
    std::vector<AppCore::UiVertex> commandTriangles;
    m_commandWindow.BuildOverlay(commandLines, commandTriangles);
    for (const AppCore::UiVertex& v : commandLines)
        m_overlayLineVertices.push_back({ v.pos, v.color });
    for (const AppCore::UiVertex& v : commandTriangles)
        m_overlayTriVertices.push_back({ v.pos, v.color });
}
...
void Application::UploadOverlayToGpu()
{
    glBindBuffer(GL_ARRAY_BUFFER, m_overlayLineVbo);
    glBufferData(GL_ARRAY_BUFFER,
        static_cast<GLsizeiptr>(m_overlayLineVertices.size() * sizeof(Vertex)),
        m_overlayLineVertices.empty() ? nullptr : m_overlayLineVertices.data(),
        GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, m_overlayTriVbo);
    glBufferData(GL_ARRAY_BUFFER,
        static_cast<GLsizeiptr>(m_overlayTriVertices.size() * sizeof(Vertex)),
        m_overlayTriVertices.empty() ? nullptr : m_overlayTriVertices.data(),
        GL_DYNAMIC_DRAW);
}
...
void Application::ZoomToWorldWindow(const glm::vec2& worldMin, const glm::vec2& worldMax, float paddingFactor)
{
    if (!m_clientSize.IsValid())
        return;

    const glm::vec2 extents = worldMax - worldMin;
    const float width = std::max(extents.x, 1.0e-3f);
    const float height = std::max(extents.y, 1.0e-3f);
    const float paddedWidth = std::max(width * paddingFactor, 1.0e-3f);
    const float paddedHeight = std::max(height * paddingFactor, 1.0e-3f);

    const float zoomX = static_cast<float>(m_clientSize.width) / paddedWidth;
    const float zoomY = static_cast<float>(m_clientSize.height) / paddedHeight;

    m_panZoom.pan = 0.5f * (worldMin + worldMax);
    m_panZoom.zoom = std::clamp(std::min(zoomX, zoomY), m_panZoom.minZoom, m_panZoom.maxZoom);
    m_needsFitToClient = false;
    m_gridCache = {};
    RebuildGridIfNeeded();
...
void Application::ZoomExtents()
{
    ...
    auto includePoint = [&](const glm::vec2& pt)
    {
        ...
        {
            worldMin = glm::min(worldMin, pt);
            worldMax = glm::max(worldMax, pt);
        }
    };
    ...
    for (const auto& ep : m_book->entities)
    {
        ...
        const auto* line = ep->As<LineEntity>();
        if (!line || !line->visible || line->tag != EntityTag::Scene || !m_layerTable.IsLayerVisible(line->layerId))
            continue;

        includePoint(glm::vec2(line->p0.x, line->p0.y));
        includePoint(glm::vec2(line->p1.x, line->p1.y));
    }

    if (!hasBounds)
        return;

    ZoomToWorldWindow(worldMin, worldMax, 1.10f);
}

bool Application::IsZoomWindowHudJigActive() const
{
    return m_hudJigRect.kind == AppCore::HudJigKind::ZoomWindow && m_hudJigRect.IsActive();
}

void Application::BeginZoomWindowHudJig()
{
    m_selection.CancelBox();
    m_pendingLeftCrossing = false;
    m_pendingRightBox = false;
    ...
    m_hudJigRect.Arm(AppCore::HudJigKind::ZoomWindow);
    m_crosshairs.SetMode(RenderCore::CrosshairsMode::LinesOnly);

    AddCommandHistory("ZOOMW: click and drag a temporary HUD jig window.");
}

void Application::CancelActiveHudJig(const char* reason)
{
    if (!m_hudJigRect.IsActive())
        return;
...
void Application::AppendGlowOverlayForLine(const LineEntity& line, const glm::vec4& color, float offsetPx)
{
    ...
    m_overlayLineVertices.push_back({ glm::vec3(a, 0.0f), color });
    m_overlayLineVertices.push_back({ glm::vec3(b, 0.0f), color });
    ...
    if (len > 1.0e-3f)
    {
        ...
        m_overlayLineVertices.push_back({ glm::vec3(a + off, 0.0f), color });
        m_overlayLineVertices.push_back({ glm::vec3(b + off, 0.0f), color });
        m_overlayLineVertices.push_back({ glm::vec3(a - off, 0.0f), color });
        m_overlayLineVertices.push_back({ glm::vec3(b - off, 0.0f), color });
    }
}
...
void Application::AppendGripOverlayForLine(const LineEntity& line, const glm::vec4& color, float halfSizePx)
{
    ...
    auto appendSquare = [&](const glm::vec2& center)
    {
        ...
        m_overlayLineVertices.push_back({ p0, color }); m_overlayLineVertices.push_back({ p1, color });
        m_overlayLineVertices.push_back({ p1, color }); m_overlayLineVertices.push_back({ p2, color });
        m_overlayLineVertices.push_back({ p2, color }); m_overlayLineVertices.push_back({ p3, color });
        m_overlayLineVertices.push_back({ p3, color }); m_overlayLineVertices.push_back({ p0, color });
    };
...
void Application::AppendHudJigOverlay()
{
    if (!IsZoomWindowHudJigActive() || !m_hudJigRect.dragging)
        return;

    const glm::vec2 a = m_panZoom.WorldToScreen(m_hudJigRect.startWorld.x, m_hudJigRect.startWorld.y);
    const glm::vec2 b = m_panZoom.WorldToScreen(m_hudJigRect.currentWorld.x, m_hudJigRect.currentWorld.y);
    const float x0 = std::min(a.x, b.x);
    const float x1 = std::max(a.x, b.x);
    const float y0 = std::min(a.y, b.y);
    const float y1 = std::max(a.y, b.y);
    const glm::vec4 color = m_zoomWindowHudJigColor;
...
void Application::OnChar(unsigned int ch)
{
    if (m_commandWindow.HandleChar(ch))
    {
        RebuildOverlayLines();
        UploadOverlayToGpu();
    }
...
void Application::OnHersheyCommandSubmitted(const std::string& commandText)
{
    AddCommandHistory("> " + commandText);

    std::string upper = commandText;
    std::transform(upper.begin(), upper.end(), upper.begin(), [](unsigned char c) { return static_cast<char>(std::toupper(c)); });

    if (upper.empty())
    {
        AddCommandHistory("(empty command)");
    }
    else if (upper == "HELP")
    {
        AddCommandHistory("Commands: HELP, CLEAR, LAYERS, PROPERTIES, RESET, SELECT NONE, BG, CROSSHAIRS, ZOOMEX, ZOOMW.");
    }
    else if (upper == "CLEAR")
    {
        m_commandWindow.ClearHistory();
        AddCommandHistory("History cleared.");
    }
    else if (upper == "LAYERS")
    {
        m_layersDialog.Toggle();
        m_layersDialog.Refresh();
        AddCommandHistory(std::string("Layers dialog ") + (m_layersDialog.IsVisible() ? "shown." : "hidden."));
    }
    else if (upper == "PROPERTIES")
    {
        m_propertiesWindow.Toggle();
        SyncPropertiesWindow();
        AddCommandHistory(std::string("Properties dialog ") + (m_propertiesWindow.IsVisible() ? "shown." : "hidden."));
    }
    else if (upper == "RESET")
    {
        ResetView();
        AddCommandHistory("View reset.");
    }
    else if (upper == "ZOOMEX")
    {
        CancelActiveHudJig(nullptr);
        ZoomExtents();
        RefreshSnappedMousePosition();
        UpdateHoveredEntity();
        SyncPropertiesWindow();
        AddCommandHistory("Zoom extents complete.");
    }
    else if (upper == "ZOOMW")
    {
        BeginZoomWindowHudJig();
    }
    else if (upper == "SELECT NONE")
    {
        m_selection.ClearSelection();
        m_selection.ClearHover();
        SyncPropertiesWindow();
        AddCommandHistory("Selection cleared.");
    }
    else if (upper == "BG")
    {
        m_crosshairs.NextBackground();
        AddCommandHistory(std::string("Background: ") + RenderCore::Crosshairs::BackgroundName(m_crosshairs.GetBackground()));
    }
    else if (upper == "CROSSHAIRS")
    {
        m_crosshairs.ToggleMode();
        AddCommandHistory(std::string("Crosshairs mode: ") + RenderCore::Crosshairs::ModeName(m_crosshairs.GetMode()));
    }
    else
    {
        AddCommandHistory("Unknown command. Type HELP.");
    }
...
void Application::RequestHersheyCommandRedraw()
{
    RebuildOverlayLines();
    UploadOverlayToGpu();
}

std::string Application::BuildCommandPrompt() const
{
    return std::string("cmd> ") + m_commandWindow.GetInputBuffer();
}

std::string Application::BuildCommandStatus() const
{
    std::ostringstream oss;
    oss << (m_commandWindow.IsFocused() ? "FOCUS " : "")
        << (m_commandWindow.IsHovered() ? "MOUSE " : "")
        << "SEL " << m_selection.GetSelectionCount();
    if (IsZoomWindowHudJigActive())
        oss << " JIG ZOOMW";
    return oss.str();
}

void Application::AddCommandHistory(const std::string& line)
{
    m_commandWindow.AddHistory(line);
}
