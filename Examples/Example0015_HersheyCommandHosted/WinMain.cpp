...
{
    Win32Console::AttachDebugConsole("Example0015_HersheyCommandHosted - Logs");

    gApplication = new Application();

    WindowCreateParams windowParams{};
    windowParams.className = L"Example0015_HersheyCommandHostedWindow";
    windowParams.title = L"Example0015_HersheyCommandHosted";
    ...
    gApplication->m_instance = hInstance;
    gApplication->m_mainHwnd = hwnd;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        SetCapture(hwnd);
        if (gApplication)
            gApplication->OnLMouseButtonDown(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
        return 0;

    case WM_LBUTTONUP:
        ReleaseCapture();
        if (gApplication)
            gApplication->OnLMouseButtonUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
        return 0;
    ...
    case WM_RBUTTONUP:
        ReleaseCapture();
        if (gApplication)
            gApplication->OnRMouseButtonUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
        return 0;
    ...
    case WM_MOUSEMOVE:
        ...
        if (gApplication)
        {
            const bool leftDown = (wParam & MK_LBUTTON) != 0;
            const bool rightDown = (wParam & MK_RBUTTON) != 0;
            const bool shiftDown = (wParam & MK_SHIFT) != 0;
            gApplication->OnMouseMove(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), leftDown, rightDown, shiftDown);
        }
        return 0;
    ...
    case LayersDialogSimple::kMsgLayerStateChanged:
        if (gApplication)
        {
            gApplication->OnLayerDialogStateChanged();
            return 0;
        }
        break;
    ...
    case WM_CHAR:
        if (gApplication)
        {
            gApplication->OnChar(static_cast<unsigned int>(wParam));
            return 0;
        }
        break;
