...
// The render path asks the window helper for the current client size,
// then hands that directly to Application so the class-oriented window code
// and the class-oriented application code stay decoupled and easy to follow.
...
{
    Win32Console::AttachDebugConsole("Example0007_GridInfiniteDragon - Logs");

    gApplication = new Application();

    WindowCreateParams windowParams{};
    windowParams.className = L"Example0007_GridInfiniteDragonWindow";
    windowParams.title = L"Example0007_GridInfiniteDragon";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        if (gApplication && wParam == 'R')
        {
            gApplication->ResetView();
            return 0;
        }
        break;
